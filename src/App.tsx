...
const CurrentRow = ({ row }: { row: AnalysisResultRow | undefined }) => (
  <section className="inspector" aria-label="Current row inspector">
    ...
    {!row ? <p className="empty-copy">No completed row is available yet. The inspector will follow the deterministic replay cursor.</p> : (
...
const App = ({ api = defaultAnalysisApi }: { api?: AnalysisApiClient }) => {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const next = await api.read(snapshot.analysisId)
        if (!active) return
        setSnapshot(next)
  ...
  return (
    <main className="analysis-shell">
      <header className="site-header"><a className="brand" href="/" aria-label="Jev Data Analysis home"><span className="brand-mark">J</span><span>JEV / DATA ANALYSIS</span></a><span className="header-tag">FIXTURE-FIRST · H1 → H2</span></header>
      <section className="hero" aria-labelledby="page-title"><div><p className="eyebrow">A bounded classifier workbench</p><h1 id="page-title">Ask a football question.<br /><em>Inspect every answer.</em></h1></div><p className="hero-copy">Draft a Jev classifier query, edit it in place, then run a transparent readback against one pinned game fixture.</p></section>
