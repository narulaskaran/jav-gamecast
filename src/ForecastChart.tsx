...
const pathFor = (points: readonly ForecastPoint[], key: keyof Pick<ForecastPoint, 'homeProbability' | 'awayProbability' | 'tieProbability'>, endIndex = points.length - 1) =>
  points.slice(0, endIndex + 1).map((point, index) => `${index === 0 ? 'M' : 'L'} ${xFor(index, points.length).toFixed(1)} ${yFor(point[key]).toFixed(1)}`).join(' ')

const SERIES = [
  { key: 'homeProbability', label: 'Harbor Hawks', short: 'Home', color: '#f05d4f' },
  { key: 'awayProbability', label: 'Cedar Foxes', short: 'Away', color: '#2f70c0' },
  { key: 'tieProbability', label: 'Tie', short: 'Tie', color: '#8b7cf6' },
] as const

export const ForecastChart = ({ points, currentIndex }: ForecastChartProps) => {
  ...
  return (
    <div className="chart-shell">
      ...
      <div className="chart-wrap">
        <svg className="forecast-chart" viewBox={`0 0 ${WIDTH} ${HEIGHT}`} role="img" aria-label="Historical forecast chart showing home, away, and tie probabilities">
          <defs>
          ...
          {SERIES.map(({ key, color }) => (
            <g key={key}>
              <path d={pathFor(points, key)} fill="none" stroke={color} strokeWidth="2" strokeDasharray="4 5" opacity=".18" className="future-line" />
              <path d={pathFor(points, key, safeIndex)} fill="none" stroke={color} strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round" className="forecast-line" />
            </g>
          ))}
              ...
              <g key={point.id} className={active ? 'chart-point is-active' : 'chart-point'}>
                ...
                {SERIES.map(({ key, color }) => (
                  <circle key={key} cx={xFor(index, points.length)} cy={yFor(point[key])} r={index === safeIndex ? 4.5 : 2.5} fill={color} stroke="#fbfaf7" strokeWidth="2">
                    <title>{`${point.timestamp}: ${key.replace('Probability', '')} ${point[key]}%`}</title>
                  </circle>
                ))}
              </g>
            )
