...
`fixtureId` remains accepted for the sample dataset. The route calls the server-only OpenRouter adapter using `OPENROUTER_KEY`. It returns an editable **Jev query JSON** object (Noul / Choice / Score) and safe metadata. Draft honors the user task. Successful drafts are reused for the same `datasetId` + normalized task (trim, collapse whitespace, case-insensitive) and are persisted in Convex `analysisDrafts` (`by_content_key`). Failed drafts and `INVALID_CLASSES` are not stored. The cached sample Noul (`Will SEA win given this play state?`) is used only for the exact default sample task (`Win likelihood of the game per play.`) on the fixture; that short-circuit is also written into the same draft cache. Any other uncached prompt calls OpenRouter. Choice drafts recover class labels from the task text (for example fruit/vehicle) and from label-like columns when the model returns fewer than two classes. Content-hash run reuse still keys only on identical `datasetId` + canonical query.
