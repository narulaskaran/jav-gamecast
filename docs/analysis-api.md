...
Temporary QA inject (unset after bash #44): on Vercel Production set `JEV_DEBUG_FAIL_AT_ROW=3` to mint a fresh run and throw a retryable `JEV_MALFORMED_RESPONSE` when classifying 0-based `rowIndex` 3 (after rows `rowIndex < 3`); empty/invalid is a no-op. Resume from the consecutive last-good prefix still classifies that row.
