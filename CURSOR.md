...
- Dataset preview shows every column (no silent column crop) in a max-height ~280px scroll box with overflow-x and overflow-y. A preview row cap is labeled `Showing first N of Y`. At 390px, table scroll stays inside the card; page width stays ≤ viewport.
