...
- Dataset preview: sample stays the full **71×26** scrollable table (no first-N cap). Large BYOD previews are bounded and/or virtualized so a ~3k×40 CSV cannot lock the main thread; the row-count badge still shows every accepted row. At 390px, table scroll stays inside the card; page width stays ≤ viewport. BYOD URL/upload shows loading progress and fails oversized or slow fetches with a plain timeout/size error.
