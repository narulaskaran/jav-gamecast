...
- Live chart is the run-view hero: Noul/Score draw a scrubbable P(win) (or score) series over play index; Choice draws class bars. Sample win-likelihood uses the full 71-play domain (denser index, same line/area, `Row X of 71`). Paired with a `Row X of Y` rail rather than a CSV inspector. Completed runs can download a formula-escaped results CSV. Never plot CSV `wpa` as if Jev produced it.
- Dataset preview shows the full accepted table (sample 71×26; BYOD all accepted rows × all columns) in a max-height ~280px scroll box with overflow-x and overflow-y. No first-N truncation caption. At 390px, table scroll stays inside the card; page width stays ≤ viewport. Sample stays fully rendered. Large BYOD tables virtualize rows so the page stays responsive. CSV intake shows loading progress; oversize or slow fetches surface a clear error instead of hanging.
