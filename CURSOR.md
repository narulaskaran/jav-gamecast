...
- Server routes under `api/analysis/*`, `api/datasets/*`, `api/share/*`, and `api/browse`. Drafting uses OpenRouter except the sample default win-likelihood draft, which is cached. Only run may call Jev, and complete snapshots are reused by content key (`datasetId` + canonical query + question kind + classes) so a second sample visitor does not mint another analysis.
