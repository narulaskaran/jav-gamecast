...
- Dataset preview shows the full accepted table (sample 71×26; BYOD all accepted rows × all columns) in a max-height ~280px scroll box with overflow-x and overflow-y. No first-N truncation caption. At 390px, table scroll stays inside the card; page width stays ≤ viewport.
- Server routes under `api/analysis/*`, `api/datasets/*`, `api/share/*`, and `api/browse`. Drafting uses OpenRouter except the sample default win-likelihood draft, which is cached. Only run may call Jev. **Same dataset + same Jev query always reuses a complete snapshot** (sample fixture and BYOD). Lookup key is `datasetId` (fixture id when that is the dataset) + canonical query + question kind + classes. A second run does not call Jev or mint another analysis. `forceNew: true` is the only escape hatch.
